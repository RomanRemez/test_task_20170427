    ...
    public byte[] generateLabel(long shipmentId) {
        ...
        try {
            ...
            if (acroForm != null) {
                ...
                field.setValue(String.valueOf(shipment.getParcels().stream().map(Parcel::getWeight)
                                                                            .reduce(0.0f, Float::sum)));

                field = (PDTextField) acroForm.getField("value");
                field.setValue(String.valueOf(shipment.getParcels().stream().map(Parcel::getDeclaredPrice)
